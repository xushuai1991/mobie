...
<script>
    ...
    export default {
        ...
        created(){
            this.$root.$on('loadorder_handle',()=>{
                if(this.list.length==0){
                    this.getOrderlist_handle(1);
                }
            });
        },
        methods: {
            getOrderlist_handle(pagenum){
                if(pagenum==1){
                    Indicator.open();
                    this.list=[];
                }
                let that=this;
                this.$http.post('/api/product/order/mall/find?pageSize=5&pageNo='+pagenum,{orderState:3})
                .then(res=>{
            ...
            loadMore() {
                ...
                if (this.allLoaded) {
                    ...
                    this.getOrderlist_handle(this.pageNum);
                }
            },
            // invoice(){
        ...
        beforeDestroy(){
            this.$root.$off('loadorder_handle');
        }
    }
</script>
<style>
